...
def main():
    ...
    if args.use_default_model:
        ...
        #Overide eval_metric if specified:      
        if not args.metric is None and not args.metric == 'None':
            print(f"Eval metric: {args.metric}")
            model_params['eval_metric'] = args.metric
        else:
            print(f"Using default Eval: {model_params['eval_metric']}")
